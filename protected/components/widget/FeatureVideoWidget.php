 ...
 * @author anhmantk
 */
class FeatureVideoWidget extends MyWidget {

    public function run() {
        $data = array();
        //danh sach feature video
        $query = "SELECT * FROM tbl_youtube WHERE is_feature = 1 ORDER BY created DESC LIMIT 5";
...
//        $data['recentVideo'] = $this->db->createCommand($query)->queryAll();
        ...
        $this->render('feature_video', array('data' => $data));
    }
